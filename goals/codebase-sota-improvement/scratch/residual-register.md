...
| CB-006 | closed (committed)     | pnpm 11.21.0 pin + exact-version frozen install proof                                                                               |
...
| RF-001 | **closed (committed)** | SOTA residual shipped on main                                                                                                       |
| RF-002 | closed (committed)     | semantic drift is enforced by `catalog:site-data:check`; intentional generation still refreshes the paired `generated_at` timestamp |
...
| -------- | --- | ----------------- | --------------------------------------------------------- |
| RV-S-001 | P1  | **closed (committed)** | Menu Button keyboard/focus model                          |
| RV-S-002 | P2  | **closed (committed)** | a11y-defer reopen → keyboard smoke                        |
| RV-S-003 | P2  | **closed (committed)** | Theme smoke gaps (System, Escape, focus)                  |
| RV-S-004 | P2  | deferred          | Component unit tests (Playwright preferred)               |
| RV-S-005 | P3  | **closed (committed)** | Tab/focus-leave dismiss                                   |
| RV-S-006 | P3  | **closed (committed)** | This register refresh                                     |
| RV-S-007 | P3  | held fence        | untracked research-upgrade interview.json + scratch logs stay unstaged |
| RV-S-008 | P2  | **closed (committed)** | Gates re-run: typecheck/lint/unit/build/**18/18** browser |

Gate evidence (2026-07-31): `pnpm web:typecheck`, `pnpm lint`, `pnpm web:test`, `pnpm web:build`, `pnpm web:test:browser` → 18 passed.

Current assurance note (2026-08-12): the dated 18-test result above remains historical. The shipped closeout adds semantic generated-data checks and an isolated, wrapper-owned browser server with no reuse; current gate counts belong to the archived `finish-web-redesign-seo-security` OpenSpec change.
...
App shell fields only: `meta.title`, `meta.repository_url`, `counts.prompts`.
